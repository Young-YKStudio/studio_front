...
const DemoFBS_Dash_Reservation_Current = ({confirmedReservations, reservationClickHandler}) => {
  return (
    <section>
      <p className="text-3xl text-white border-b-2 border-double pb-2 mt-4 mb-6 font-bold truncate lg:text-center">Current Reservations</p>
      { confirmedReservations &&
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 mr-4">
          {confirmedReservations && confirmedReservations.map((reservation) => (
            <div
              ...
              >
              <p className='text-lg text-green-800 font-bold'>{reservation.reservedName}</p>
...
export default DemoFBS_Dash_Reservation_Current;